...
def test_upsert_then_known(store: Store):
    store.upsert_listing(make_listing("1"), "fp1", "track-loaders", "bobcat-t770")
...
def test_known_listing_ids_handles_large_batches(store: Store):
    ...
    for i in range(1200):
        store.upsert_listing(make_listing(str(i)), f"fp{i}")
...
def test_upsert_preserves_first_seen_and_updates_last_seen(store: Store):
    store.upsert_listing(make_listing("1"), "fp1")
...
def test_fingerprint_seen_before_ignores_the_listing_itself(store: Store):
    store.upsert_listing(make_listing("1"), "fp-shared")
...
def test_fingerprint_seen_before_detects_a_repost(store: Store):
    store.upsert_listing(make_listing("1"), "fp-shared")
    store.upsert_listing(make_listing("2"), "fp-shared")
...
def test_store_works_as_context_manager(tmp_path: Path):
    with Store(tmp_path / "cm.db") as s:
        s.initialize()
        s.upsert_listing(make_listing("1"), "fp1")
...
def test_v1_migration_backfills_a_half_populated_row(tmp_path):
    """A migration interrupted between the two columns leaves a row with a
    model but no watchlist. Guarding on `model_name IS NULL` alone would skip
    it forever, stranding it outside every watchlist."""
    ...
    conn.executescript(
        ...
        INSERT INTO schema_version (version) VALUES (1);
        CREATE TABLE listings (
            listing_id TEXT PRIMARY KEY, search_name TEXT NOT NULL,
        ...
        INSERT INTO listings (listing_id, search_name, model_name, title, url,
                              fingerprint, first_seen_at, last_seen_at)
        VALUES ('a', 'bobcat-t770', 'bobcat-t770', '2019 Bobcat T770',
                'u', 'f', 't', 't');
        """
    )
    conn.commit()
...
def test_v2_database_drops_search_name(tmp_path: Path):
    """v3 retires the column that recorded which query claimed a listing.
    Pooling made it meaningless — every query feeds every model."""
    path = tmp_path / "v2.db"
    conn = sqlite3.connect(path)
    conn.executescript(
        """
        ...
        VALUES ('a', 'bobcat-t86', 'track-loaders', 'bobcat-t770',
...
def test_reassign_updates_watchlist_and_model(store: Store):
    store.upsert_listing(make_listing("1"), "fp")
